...
function handlePaymentChange(e) {
  ...
  if (method === 'Bkash') {
    ...
    noteEl.textContent = `Pay delivery charge ৳${document.getElementById('co-delivery').dataset.fee} to ${COD_NUMBER} via bKash. Remaining on delivery.`;
    txnEl.required = true; // REQUIRED FOR COD
    txnEl.value = '';
    payNowEl.style.display = 'block';
    dueEl.style.display = 'block';
  } else {
...
function updateTotalInModal() {
  ...
  if (paymentMethod === 'Bkash') {
    ...
    // Update note dynamically if address changes and COD is selected
    const noteEl = document.getElementById('co-note');
    if (noteEl) {
...
async function submitCheckoutOrder(e) {
  ...
  // TRANSACTION ID REQUIRED FOR BOTH BKASH AND COD
  ...
  document.getElementById('checkout-cart')?.addEventListener('click', async () => {
    ...
    function updateCartCheckoutTotals() {
      ...
      if (hasPreOrder) {
        ...
        noteEl.textContent = `Pay delivery charge ৳${deliveryFee} via bKash. Remaining on delivery.`;
        txnEl.required = true; // REQUIRED FOR COD IN CART CHECKOUT
        txnEl.value = '';
      } else {
        numberEl.value = '';
        noteEl.textContent = '';
